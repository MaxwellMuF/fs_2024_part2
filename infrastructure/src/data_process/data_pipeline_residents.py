...
def data_process_res(df_geodat_plz: pd.DataFrame, df_plz_res: pd.DataFrame): #, required_columns: tuple
    """Preprocessing dataframe from plz_einwohner.csv"""
    df_residents                    = df_plz_res.copy()
  
    
    # Select only important columns and rename them
    df_residents                    = df_residents.loc[:,['plz', 'einwohner', 'lat', 'lon']]
    df_residents.rename(columns     = {"plz": "PLZ", "einwohner": "Einwohner", "lat": "Breitengrad", "lon": "Längengrad"}, 
                        inplace     = True)

    # Convert to string
    df_residents['Breitengrad']     = df_residents['Breitengrad'].astype(str)
    df_residents['Längengrad']      = df_residents['Längengrad'].astype(str)

    # Now replace the commas with periods
    df_residents['Breitengrad']     = df_residents['Breitengrad'].str.replace(',', '.')
    df_residents['Längengrad']      = df_residents['Längengrad'].str.replace(',', '.')

    # Filter criteria "PLZ": only select "PLZ" in berlin
    df_residents                    = df_residents[(df_residents["PLZ"] > 10115) &
                                                (df_residents["PLZ"] < 14200)].copy().astype({"PLZ" : int})
    
    # Sort df by "PLZ" and reset index
    df_residents.sort_values(by     ="PLZ", inplace=True, ignore_index=True)
    
    # Merge df_residents and df_geodat_plt to get geo data for berlins zip codes
    df_residents                    = df_residents.merge(df_geodat_plz, on=pdict["geocode"], how ='left')
    df_residents.dropna(subset      =['geometry'], inplace=True)

    # Make geopanda df for creationg heatmap later on
    df_residents.loc[:,'geometry']  = gpd.GeoSeries.from_wkt(df_residents['geometry'])
    df_residents                    = gpd.GeoDataFrame(df_residents, geometry='geometry')
